...
public class ImageService(IConfiguration configuration) : IImageService
{
    ...
    {
        var sizes = configuration.GetRequiredSection("Images:Sizes").Get<List<int>>();
        var dir = Path.Combine(Directory.GetCurrentDirectory(), configuration["Images:Dir"]!);
    ...
    {
        var path = Path.Combine(Directory.GetCurrentDirectory(), configuration["Images:Dir"]!,
            $"{size}_{name}");
